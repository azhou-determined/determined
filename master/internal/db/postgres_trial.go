...
func (db *PgDB) _addTrialMetricsTx(
	...
	if m.StepsCompleted != nil {
		stepsCompleted = *m.StepsCompleted
	}

	if rollbacks, err = rollbackMetrics(ctx, tx, m.TrialRunId, m.TrialId, stepsCompleted,
		mGroup); err != nil {
		return rollbacks, err
	}
	...
	metricRowID, addedMetrics, err := db.addMetricsWithMerge(ctx, tx,
		mBody, m.TrialRunId, m.TrialId, stepsCompleted, mGroup)
	if err != nil {
		return rollbacks, err
	}
...
WHERE id = $1;
`, m.TrialId, stepsCompleted, summaryMetrics, latestValidationID); err != nil {
...
func (db *PgDB) _addTrialProfilingMetricsTx(
	...
	_, err = db.addRawMetrics(ctx, tx,
		mBody, m.TrialRunId, m.TrialId, nil, mGroup)
